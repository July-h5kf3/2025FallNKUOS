...
- 一是将当前的控制寄存器 `sscratch` 中的值读到寄存器`s0`中，相当于寄存器`s0`保存了上下文切换前的栈顶指针，然后后续将该寄存器保存到栈中，这样trap处理函数的时候或后续回复的时候，都可以取到原来的用户栈。
- 二是将寄存器`sscratch`写为0，之所以要对控制寄存器进行清零操作，是因为如果再次发生嵌套trap(例如内核代码执行时发生系统调用)，那么trap的入口代码就会对寄存器`sscratch`的值进行识别，若此时该寄存器中值为零，那么就可以确定当前trap是来自内核态而非用户态，这样就会依照S模式触发中断的路由进行，也就不会再一次切换栈。
