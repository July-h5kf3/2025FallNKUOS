...
load_icode(int fd, int argc, char **kargv)
{
    ...
    if ((ret = load_icode_read(fd, phdrs, sizeof(struct proghdr) * elf.e_phnum, elf.e_phoff)) != 0)
    {
        goto bad_free_phdrs;
    }
    ...
    {
        ret = -E_NO_MEM;
        uargv = kmalloc(sizeof(uintptr_t) * (argc + 1));
        if (uargv == NULL)
        {
            goto bad_cleanup_user;
        }
        int i;
        for (i = argc - 1; i >= 0; i--)
        {
